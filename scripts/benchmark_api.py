#!/usr/bin/env python3
"""Run and preserve the local Docker HTTP benchmark matrix."""
...
def _request(
    ...
    request_id = f"api-benchmark-{scenario}-{uuid.uuid4().hex}"
...
def _readiness(base_url: str, expected_backend: str) -> dict[str, Any]:
    ...
    if "fake" in str(body.get("embedding_backend", "")).lower():
        raise RuntimeError("fake embedding backend is forbidden in API benchmarks")
    return body
...
def _run_profile(args: argparse.Namespace) -> None:
    ...
    if args.requests < 100:
        raise ValueError("recorded benchmark scenarios require at least 100 requests")
