#!/usr/bin/env python3
"""Run the frozen Phase 8 WANDS test split across real retrieval backends."""
...
OUTPUT_DIR = Path("data/evaluation/phase8_wands")
MANIFEST_PATH = Path("reports/phase8_run_manifest.json")
...
def _evaluate(
    ...
    # This run intentionally retrieves TOP_K=10. The shared Phase 4 helper
...
def _load_manifest() -> dict[str, Any]:
    ...
    if actual_hash != manifest["config_hash"]:
        raise RuntimeError("Phase 8 manifest config hash mismatch")
    return manifest
    ...
    try:
        ...
        result = {
            ...
            "historical_holdout_limitation": (
                "Phase 4 evaluated all 480 WANDS queries before this split was defined; "
                "the Phase 8 test run is frozen and not used for retuning, but is not a "
