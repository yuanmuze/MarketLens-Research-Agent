"""Tests for the recommendation orchestrator, tools, evidence, and providers."""
...
class TestToolResultSourceVerification:
    ...
    def test_max_results_limits_final_recommendations(
        self,
        service: RetrievalService,
    ) -> None:
        """The public max_results contract limits tool-derived output."""
        fake = _make_fake_llm(
            tool_calls=[{
                "id": "c1",
                "type": "function",
                "function": {
                    "name": "search_catalog",
                    "arguments": json.dumps({"query": "headphones", "top_k": 5}),
                },
            }],
            final_content="Done.",
        )
        ...
        response = orchestrator.run(
            AgentRequest(message="headphones", max_results=2),
        )

        assert len(response.recommendations) == 2
        assert {item.product_id for item in response.evidence} == {
