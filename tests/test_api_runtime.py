"""API configuration, dependency injection, and lifespan tests."""
...
def test_fake_llm_is_explicit_and_deterministic() -> None:
    ...
    assert client.model_name == "marketlens-offline-deterministic"
...
def test_request_id_propagates_to_search_response() -> None:
    ...
    assert response.json()["request_id"] == "runtime-request-id"
...
def test_unavailable_service_returns_structured_503() -> None:
    ...
    response = client.get(
        "/search?q=headphones",
        headers={"X-Request-ID": "runtime-unavailable"},
    )
    ...
    assert response.json() == {
        ...
        "request_id": "runtime-unavailable",
    }
...
def test_liveness_survives_startup_dependency_failure(
    ...
    with TestClient(app) as client:
        ...
        ready = client.get(
            "/health/ready",
            headers={"X-Request-ID": "runtime-not-ready"},
        )

    assert live.status_code == 200
    assert ready.status_code == 503
    assert ready.json()["code"] == "service_unavailable"
    assert ready.json()["request_id"] == "runtime-not-ready"
